    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for ii, train_data in enumerate(train_loader):
            ...
            if rank <= 0:
                ...
                message = "ep={}, iter={}/{}, lr={:6f}".format(epoch, current_step, total_iters, model.get_current_learning_rate())
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0 and val_loader is not None:
                ...
                idx = 0
                print("\n\tvalidation: ep={}, curr_step={}".format(epoch, current_step))
                n_test_pick = len(val_set.test_samples)
                for ti in range(n_test_pick):
                    test_sample = val_set.get_test_sample(ti)
                    img_name = "TEST_" + os.path.splitext(os.path.basename(test_sample['LQ_path'][0]))[0]
